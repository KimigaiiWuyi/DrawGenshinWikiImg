...
async def draw_artifacts_wiki_img(data: dict) -> None:
    # 获取数据
    ...
    for index, i in enumerate(artifacts_list):
        ...
        artifacts_part_img = Image.open(BytesIO(get(icon_url).content)).convert(
            'RGBA').resize((80, 80), Image.Resampling.LANCZOS)
        # artifacts_part_img = Image.open(
        # RELIC_PATH / icon).convert('RGBA').resize((80, 80), Image.Resampling.LANCZOS)
        artifacts_bar.paste(artifacts_part_img, (76, 32 +
                            index * 90), artifacts_part_img)
        artifacts_bar_draw.text(
            (183, 58 + 90 * index), data[i]['name'], (154, 123, 51), genshin_font_origin(26), 'lm')
        artifacts_bar_draw.text((183, 90 + 90 * index), fill(
            data[i]['description'], width=24), (182, 173, 165), genshin_font_origin(14), 'lm')
    ...
    text_draw.text((295, 182), data['name'],
                   (154, 123, 51), genshin_font_origin(40), 'mm')
    text_draw.text((295, 230), rarity, (175, 145, 75),
                   genshin_font_origin(23), 'mm')
    if '1pc' in data:
        text_draw.text((63, 250 + 10 + 3), pc1,
                       (111, 100, 80), genshin_font_origin(22))
    else:
        text_draw.text((63, 250 + 10 + 3), pc2,
                       (111, 100, 80), genshin_font_origin(22))
        text_draw.text((63, 250 + y1 + 40 + 3), pc4,
                       (111, 100, 80), genshin_font_origin(22))
    ...
    result_img.save(WIKI_ARTIFACTS_PATH / '{}.jpg'.format(
        data['name']), format='JPEG', quality=100, subsampling=0)
    return None


async def draw_weapons_wiki_img(data: dict) -> None:
    # 获取数据
    ...
    attr = '攻击力 {}/{}   {} {}/{}'.format(base_atk,
                                         atk, sub_name, base_sub_val, sub_val)
    ...
    # 贴图
    weapon_img = Image.open(BytesIO(get(data['images']['icon']).content)).convert(
        'RGBA').resize((312, 312), Image.Resampling.LANCZOS)
    ...
    for index, cost in enumerate(costs):
        ...
        if index < 1:
            cost_info = cost_data['dropdomain'] + \
                '({})'.format(' & '.join(cost_data['daysofweek'][:-1]))
        ...
        print(r.url)
        cost_icon = Image.open(BytesIO(r.content)).convert(
            'RGBA').resize((90, 90), Image.Resampling.LANCZOS)
        ...
        cost_draw.text(
            (64, 115), cost_data['name'], (255, 255, 255), genshin_font_origin(13), 'mm')
        result_img.paste(
            cost_bg, (30 + index*140, 660 + 100 + y + 110), cost_bg)

    # 写字
    text_draw = ImageDraw.Draw(result_img)
    text_draw.text((295, 579), data['name'],
                   (154, 123, 51), genshin_font_origin(40), 'mm')
    text_draw.text((295, 632), '稀有度：' +
                   data['rarity'], (175, 145, 75), genshin_font_origin(23), 'mm')
    text_draw.text((295, 695), attr, (255, 255, 255),
                   genshin_font_origin(28), 'mm')

    text_draw.text((40, 660 + 50 + y + 120), cost_info,
                   (175, 145, 75), genshin_font_origin(28), 'lm')

    if data['effectname']:
        text_draw.text((295, 750), data['effectname'],
                       (175, 145, 75), genshin_font_origin(24), 'mm')
        text_draw.text((38, 660+120), effect, (87, 89, 101),
    ...
    result_img.save(WIKI_WEAPON_PATH / '{}.jpg'.format(
        data['name']), format='JPEG', quality=100, subsampling=0)

    return None


async def draw_chars_wiki_img(data: dict) -> None:
    # 获取数据
    ...
    charimg = Image.open(BytesIO(get(data['images']['cover1']).content)).convert(
        'RGBA').resize((862, 528), Image.Resampling.LANCZOS)
    ...
    skill_costs = [skill_data['costs']['lvl9']
                   [1], skill_data['costs']['lvl9'][-1]]
    ...
    for index, cost in enumerate(costs):
        ...
        cost_draw.text(
            (64, 115), cost_data['name'], (255, 255, 255), genshin_font_origin(13), 'mm')
        result_img.paste(cost_bg, (30 + (index % 3)*140, 360 +
                         580 + 55 + y_length + (index//3 * 140)), cost_bg)
    ...
    for skill in y:
        text_draw.rounded_rectangle(
            (32, y1, 558, y1 + skill['y'] + 25), fill=(220, 211, 203), radius=15)
        text_draw.text(
            (48, y1+25), skill['name'], (194, 155, 78), genshin_font_origin(23), 'lm')
        text_draw.text((48, y1 + (skill['y'] + 10)/2 + 20),
                       skill['info'], (69, 69, 69), genshin_font_origin(23), 'lm')
        y1 += skill['y'] + 50

    text_draw.text((295, 602), data['name'],
                   (154, 123, 51), genshin_font_origin(40), 'mm')
    text_draw.text((295, 635), '稀有度：' +
                   data['rarity'], (175, 145, 75), genshin_font_origin(23), 'mm')
    ...
    result_img.save(
        WIKI_CHAR_PATH / '{}.jpg'.format(data['name']), format='JPEG', quality=100, subsampling=0)
    return None
...
async def get_artifacts_wiki_img(_list: list) -> None:
    ...
    for item in _list:
        ...
        # 获取数据
        data = await get_misc_info('artifacts', item)
        name = data['name']
        resource_temp_path = WIKI_ARTIFACTS_PATH/f'{name}.jpg'
        if resource_temp_path.exists():
            print(f'本地已有{name}的资源')
            print(resource_temp_path)
        else:
            print('本地未缓存,正在渲染...')
            print(f'正在导出{name}……')
            await draw_artifacts_wiki_img(data)
            i += 1
    print(f'累计导出{i}张')
...
async def get_weapons_wiki_img(_list: list) -> None:
    ...
    for item in _list:
        ...
        # 获取数据
        data = await get_weapon_info(item)
        name = data['name']
        ...
        if resource_temp_path.exists():
            ...
            print('本地未缓存,正在渲染...')
            print(f'正在导出{name}……')
            await draw_weapons_wiki_img(data)
            i += 1
    print(f'累计导出{i}张')
...
async def get_chars_wiki_img(_list: list) -> None:
    ...
    for item in _list:
        ...
        # 获取数据
        data = await get_char_info(item, 'char', None)
        name = data['name']
        ...
        if resource_temp_path.exists():
            ...
            print('本地未缓存,正在渲染...')
            print(f'正在导出{name}……')
            await draw_chars_wiki_img(data)
            i += 1
    print(f'累计导出{i}张')
