...
async def draw_artifacts_wiki_img(name: str) -> None:
    # 获取数据
    ...
    for index, i in enumerate(data['images']):
        ...
        artifacts_part_img = Image.open(RELIC_PATH / icon).convert('RGBA').resize((80, 80), Image.Resampling.LANCZOS)
        artifacts_bar.paste(artifacts_part_img, (76, 32 + index * 90), artifacts_part_img)
        artifacts_bar_draw.text((183, 58 + 90 * index), data[i]['name'], (154, 123, 51), genshin_font_origin(26), 'lm')
        artifacts_bar_draw.text((183, 90 + 90 * index), fill(data[i]['description'], width = 24), (182, 173, 165), genshin_font_origin(14), 'lm')
    ...
    text_draw.text((295, 182), data['name'], (154, 123, 51), genshin_font_origin(40), 'mm')
    text_draw.text((295, 230), rarity, (175, 145, 75), genshin_font_origin(23), 'mm')
    if '1pc' in data:
        text_draw.text((63, 250 + 10 + 3), pc1, (111, 100, 80), genshin_font_origin(22))
    else:
        text_draw.text((63, 250 + 10 + 3), pc2, (111, 100, 80), genshin_font_origin(22))
        text_draw.text((63, 250 + y1 + 40 + 3), pc4, (111, 100, 80), genshin_font_origin(22))
    ...
    result_img.save(WIKI_ARTIFACTS_PATH / '{}.jpg'.format(data['name']), format='JPEG', quality=100, subsampling=0)
    return None

async def draw_weapons_wiki_img(name: str) -> None:
    # 获取数据
    ...
    if data['subvalue'] != '':
        base_sub_val = (data['subvalue'] +
                    '%') if sub_name != '元素精通' else data['subvalue']
    else:
        base_sub_val = ''

    if sub_name != '':
        sub_val = '%.1f%%' % (level90_data['specialized'] * 100) \
            if sub_name != '元素精通' else str(round(level90_data['specialized']))
    else:
        sub_val = ''
    atk = str(round(level90_data['attack']))

    attr = '攻击力 {}/{}   {} {}/{}'.format(base_atk, atk, sub_name, base_sub_val, sub_val)
    ...
    # 贴图
    weapon_img = Image.open(BytesIO(get(data['images']['icon']).content)).convert('RGBA').resize((312, 312), Image.Resampling.LANCZOS)
    ...
    for index,cost in enumerate(costs):
        cost_data = json.loads(get('https://info.minigg.cn/materials?query=' + cost['name']).text)
        if index < 1:
            cost_info = cost_data['dropdomain'] + '({})'.format(' & '.join(cost_data['daysofweek'][:-1]))
        cost_bg = Image.open(cost_bg_path)
        cost_icon = Image.open(ITEM_PATH / '{}.png'.format(cost_data['images']['nameicon'])).resize((90, 90), Image.Resampling.LANCZOS)
        cost_bg.paste(cost_icon,(15,10),cost_icon)
        cost_draw = ImageDraw.Draw(cost_bg)
        cost_draw.text((64, 115), cost_data['name'], (255, 255, 255), genshin_font_origin(13), 'mm')
        result_img.paste(cost_bg, (30 + index*140, 660 + 100 + y + 110), cost_bg)

    # 写字
    text_draw = ImageDraw.Draw(result_img)
    text_draw.text((295, 579), data['name'], (154, 123, 51), genshin_font_origin(40), 'mm')
    text_draw.text((295, 632), '稀有度：' + data['rarity'], (175, 145, 75), genshin_font_origin(23), 'mm')
    text_draw.text((295, 695), attr, (255, 255, 255), genshin_font_origin(28), 'mm')
    
    text_draw.text((40, 660 + 50 + y + 120), cost_info, (175, 145, 75), genshin_font_origin(28), 'lm')

    if data['effectname']:
        text_draw.text((295, 750), data['effectname'], (175, 145, 75), genshin_font_origin(24), 'mm')
        text_draw.text((38, 660+120), effect, (87, 89, 101), genshin_font_origin(24))
    ...
    result_img.save(WIKI_WEAPON_PATH / '{}.jpg'.format(data['name']), format='JPEG', quality=100, subsampling=0)
    
    return None

async def draw_chars_wiki_img(name: str) -> None:
    # 获取数据
    ...
    charimg = Image.open(BytesIO(get(data['images']['cover1']).content)).convert('RGBA').resize((862,528), Image.Resampling.LANCZOS)
    ...
    for skill in skill_data:
        if skill.startswith('passive'):
            ...
            _,_,_,y_temp = img_draw.textbbox((0, 0), skill_name + '\n' + skill_info, genshin_font_origin(23))
    ...
    skill_costs = [skill_data['costs']['lvl9'][1], skill_data['costs']['lvl9'][-1]]
    ...
    for index,cost in enumerate(costs):
        ...
        cost_draw = ImageDraw.Draw(cost_bg)
        cost_draw.text((64, 115), cost_data['name'], (255, 255, 255), genshin_font_origin(13), 'mm')
        result_img.paste(cost_bg, (30 + (index%3)*140, 360 + 580 + 55 + y_length + (index//3 * 140)), cost_bg)
    ...
    y1 = 950
    for skill in y:
        text_draw.rounded_rectangle((32, y1, 558, y1 + skill['y'] + 25), fill=(220, 211, 203), radius = 15)
        text_draw.text((48, y1+25), skill['name'], (194, 155, 78), genshin_font_origin(23), 'lm')
        text_draw.text((48, y1 + (skill['y'] + 10)/2 + 20), skill['info'], (69, 69, 69), genshin_font_origin(23), 'lm')
        y1 += skill['y'] + 50

    text_draw.text((295, 602), data['name'], (154, 123, 51), genshin_font_origin(40), 'mm')
    text_draw.text((295, 635), '稀有度：' + data['rarity'], (175, 145, 75), genshin_font_origin(23), 'mm')
    text_draw.text((295, 912), await strLenth(data['description'], 23, 495), (69, 69, 69), genshin_font_origin(23), 'mm')
    #text_draw.text((295, 695), attr, (255, 255, 255), genshin_font_origin(28), 'mm')
    ...
    result_img.save(WIKI_CHAR_PATH / '{}.jpg'.format(data['name']), format='JPEG', quality=100, subsampling=0)
    return None

async def main():
    a_list=['海染','勇士','辰砂往生录','来歆余响','华馆梦醒形骸记','海染砗磲','绝缘之旗印','追忆之注连','千岩牢固','苍白之火','平息鸣雷的尊者','炽烈的炎之魔女','流浪大地的乐团','染血的骑士道','被怜爱的少女','角斗士的终幕礼','渡过烈火的贤人','悠古的磐岩','如雷的盛怒','沉沦之心','逆飞的流星','昔日宗室之仪','翠绿之影','冰风迷途的勇士','勇士之心','教官','流放者','战狂','武人','学士','赌徒','奇迹','行者之心','守护之心','幸运儿','冒险家','游医', '祭冰之人','祭雷之人','祭火之人','祭水之人']
    b_list=['飞雷']
    c_list=['钟离']

    #for i in a_list:
    #    print('正在导出{}……'.format(i))
    #    await draw_artifacts_wiki_img(i)
    
    #for i in b_list:
    #    print('正在导出{}……'.format(i))
    #    await draw_weapons_wiki_img(i)

    for i in c_list:
        print('正在导出{}……'.format(i))
        await draw_chars_wiki_img(i)
